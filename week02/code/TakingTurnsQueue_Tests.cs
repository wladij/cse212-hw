...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found: The queue did not correctly handle remaining turns before re-enqueuing people.
    ...
    // Defect(s) Found: Players were not re-enqueued correctly after each turn, causing the queue order to become incorrect.
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: Players with 0 turns (infinite turns) were removed from the queue instead of being re-enqueued.
    ...
    // Defect(s) Found: Players with negative turns (infinite turns) were not treated as having infinite turns.
